...
func TestBaseOrchestrationBuilder_ApplyConfigurationInheritance_LocalOverrideAndParentIntact(t *testing.T){
	...
	if final.Timeout != 10*time.Second { t.Errorf("expected override timeout 10s got %v", final.Timeout) }
	if final.MaxConcurrency != 3 { t.Errorf("expected inherited max concurrency 3 got %d", final.MaxConcurrency) }
	if parent.Timeout != 5*time.Second { t.Errorf("parent mutated timeout %v", parent.Timeout) }
}
...
func TestBaseOrchestrationBuilder_ApplyConfigurationInheritance_ErrorBoundaryOverrideExisting(t *testing.T){
	...
	final := child.ApplyConfigurationInheritance(parent)
	if final.ErrorStrategy != errors.CollectAll { t.Errorf("expected boundary override CollectAll got %v", final.ErrorStrategy) }
}
...
func TestBaseOrchestrationBuilder_ApplyConfigurationInheritance_NoLocalConfig(t *testing.T){
	...
	final := child.ApplyConfigurationInheritance(parent)
	if final.Timeout != 2*time.Second || final.MaxConcurrency != 9 || final.ErrorStrategy != errors.CollectAll { t.Errorf("inheritance failed: %+v", final) }
}
