...
func TestConcurrent_Execute_FailFast_Success(t *testing.T){
	...
	if res.Get("one")!=1 || res.Get("two")!=2 { t.Error("unexpected results") }
}
...
func TestConcurrent_Execute_FailFast_FirstError(t *testing.T){
	...
	c := Concurrent(
		...
		task.Task(func()(int,error){ atomic.AddInt32(&secondStarted,1); time.Sleep(100*time.Millisecond); return 42,nil }),
	).ErrorBoundary(internalErrors.FailFast)
	res, err := c.Execute(context.Background(), config.DefaultConfig())
	if err==nil { t.Fatal("expected error") }
	if len(res.Errors())==0 { t.Fatalf("expected at least one error") }
	if res.Errors()[0].Error.Error() != "boom" { t.Errorf("first error should be boom, got %v", res.Errors()[0].Error) }
}
...
func TestConcurrent_Execute_FailFast_Cancellation(t *testing.T){
	...
	if err==nil { t.Error("expected cancellation error") }
	if atomic.LoadInt32(&started)==0 { t.Error("first task not started") }
}
