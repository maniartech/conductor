...
func TestConcurrent_Execute_CollectAll_Mixed(t *testing.T){
	...
	if err==nil { t.Fatal("expected aggregated error") }
	if len(res.Errors())!=2 { t.Errorf("expected 2 errors got %d", len(res.Errors())) }
	if res.Get("a")!="A" || res.Get("c")!=true { t.Error("expected successful results retained") }
}
...
func TestConcurrent_Execute_CollectAll_Panic(t *testing.T){
	...
	res, err := c.Execute(context.Background(), config.DefaultConfig())
	if err==nil { t.Fatal("expected panic aggregated error") }
	if len(res.Errors())!=1 { t.Errorf("expected 1 error got %d", len(res.Errors())) }
	if res.Get("a")!="A" || res.Get("c")!="C" { t.Error("other results missing") }
}
...
func TestConcurrent_Execute_CollectAll_Timeout(t *testing.T){
	...
	if err==nil { t.Error("expected timeout error") }
}
