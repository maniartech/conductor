...
// ErrorContext provides rich context information for sequential orchestration errors.
// It captures detailed information about the execution environment when errors occur,
// enabling comprehensive debugging and observability.
type ErrorContext struct {
	SequentialName      string                        // Name of the sequential orchestration
	SequentialID        string                        // Unique ID of the sequential orchestration
	TotalSteps          int                           // Total number of steps in the sequential
	...
	ErrorStrategy       errors.ErrorStrategy          // Error strategy being used
	ErrorBoundary       *errors.ErrorStrategy         // Error boundary if set
	ContextCancelled    bool                          // Whether context was cancelled
	StackTrace          []byte                        // Stack trace at error point
	ChildOrchestrations []orchestration.Orchestration // Child orchestrations for reference
}

// ErrorBoundaryHandler manages error boundary containment within sequential blocks.
...
// NewErrorBoundaryHandler creates a new error boundary handler for a sequential orchestration.
...
func NewErrorBoundaryHandler(strategy errors.ErrorStrategy, boundaryName string, parentContext *ErrorContext) *ErrorBoundaryHandler {
	return &ErrorBoundaryHandler{
		strategy:      strategy,
		boundaryName:  boundaryName,
		parentContext: parentContext,
		allErrors:     make([]errors.OperationError, 0),
	}
}
...
//
// Returns:
//   - error: The final error to return from the sequential orchestration
//
// Example:
//
...
func (ebh *ErrorBoundaryHandler) GetFinalError() error {
	...
	switch ebh.strategy {
	case errors.FailFast:
		return fmt.Errorf("sequential orchestration failed in error boundary '%s': %w", ebh.boundaryName, ebh.firstError)
	case errors.CollectAll:
		return fmt.Errorf("sequential orchestration completed with %d errors in error boundary '%s', first error: %w",
...
//
// Returns:
//   - []errors.OperationError: All errors that occurred within the boundary
func (ebh *ErrorBoundaryHandler) GetAllErrors() []errors.OperationError {
	// Return a copy to prevent external modification
	errorsCopy := make([]errors.OperationError, len(ebh.allErrors))
...
// CreateErrorContext creates a rich error context for a sequential orchestration.
...
//   - orchestrations: Child orchestrations
//   - errorStrategy: Error strategy being used
//   - errorBoundary: Error boundary if set
//
// Returns:
//   - *ErrorContext: Rich error context information
func CreateErrorContext(sequentialName, sequentialID string, orchestrations []orchestration.Orchestration,
	errorStrategy errors.ErrorStrategy, errorBoundary *errors.ErrorStrategy) *ErrorContext {

	return &ErrorContext{
		SequentialName:      sequentialName,
		SequentialID:        sequentialID,
		TotalSteps:          len(orchestrations),
		CompletedSteps:      0,
		FailedStep:          -1,
		FailedStepName:      "",
		ExecutionTime:       0,
		...
		StackTrace:          nil,
		ChildOrchestrations: orchestrations,
	}
}
...
Error Details:
`,
		eer.context.SequentialName,
		eer.context.SequentialID,
