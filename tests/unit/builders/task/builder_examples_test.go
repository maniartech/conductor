...
func ExampleTask() {
	tk := Task(func(ctx orchestrator.Context) (string, error) { return "Hello, World!", nil })
...
func ExampleTaskBuilder_Named() {
	tk := Task(func(ctx orchestrator.Context) (int, error) { return 42, nil }).Named("answer-task")
	res, err := tk.Execute(context.Background(), config.DefaultConfig())
	if err != nil {
		fmt.Printf("Error: %v\n", err)
		return
